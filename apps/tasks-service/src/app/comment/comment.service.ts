  ...
  async create(createTask: CreateCommentDto, userID: string) {
    ...
    try {
      const task = await this.taskRepository.findOne({
        where: { id: createTask.taskId },
      });
      this.rabbitMQService.publishCommentCreated(newComment, task);
      Logger.log(
        `Published task.comment.created event for comment ${newComment.id}`,
      );
    } catch (error) {
      Logger.error(
        `Failed to publish task.comment.created event for comment ${newComment.id}:`,
        error,
      );
    }
