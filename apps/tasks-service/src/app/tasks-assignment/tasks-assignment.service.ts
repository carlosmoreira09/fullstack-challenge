...
import {CreateTaskAssignmentDto, UpdateTaskAssignmentDto} from "@taskmanagerjungle/types";
...
export class TasksAssignmentService {
    ...
    async findOneByTask(taskId: string) {
        return await this.taskAssignmentRepository.findOne({ where: { taskId: taskId } });
    }
    async findByTask(taskId: string) {
        return await this.taskAssignmentRepository.find({ where: { taskId: taskId } });
    }

    async findByAssigned(assignedId: string) {
        return await this.taskAssignmentRepository.find({ where: { assignedById: assignedId } });
    }
    async findByUser(userId: string) {
        return await this.taskAssignmentRepository.find({ where: { userId: userId } });
    }
    ...
    async create(createAssignment: CreateTaskAssignmentDto) {
        const taskAssignment = this.taskAssignmentRepository.create(createAssignment);
    ...
    async update(updateAssignment: UpdateTaskAssignmentDto) {
        ...
        const updateObject = this.taskAssignmentRepository.create({...assignment,...updateAssignment})
