...
export class AppService {
  ...
  async create(createTask: CreateTaskDto) {
    ...
    if (newTask) {
      ...
      try {
        this.rabbitMQService.publishTaskCreated(newTask);
        Logger.log(`Published task.created event for task ${newTask.id}`);
      } catch (error) {
        Logger.error(
          `Failed to publish task.created event for task ${newTask.id}:`,
          error,
        );
      }
    }
  ...
  async update(id: string | undefined, updateTask: UpdateTaskDto) {
    ...
    if (
      updateTask.description !== undefined &&
    ...
    if (
      ...
      JSON.stringify(updateTask.assignees) !== JSON.stringify(task.assignees)
    ) {
      ...
      for (const assignee of removedAssignees) {
        try {
          const assignments = await this.taskAssignmentService.findByTask(
            task.id,
          );
          const assignment = assignments.find(
            (a) => a.userId === assignee && a.isActive,
          );
    ...
    if (Object.keys(changes).length > 0) {
      ...
      try {
        this.rabbitMQService.publishTaskUpdated(updatedTask);
        Logger.log(`Published task.updated event for task ${updatedTask.id}`);
      } catch (error) {
        Logger.error(
          `Failed to publish task.updated event for task ${updatedTask.id}:`,
          error,
        );
      }
    }
