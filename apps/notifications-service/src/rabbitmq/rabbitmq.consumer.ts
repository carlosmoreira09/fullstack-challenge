...
export class RabbitMQConsumer implements OnModuleInit, OnModuleDestroy {
  ...
  private async setupConsumers() {
    try {
      await this.channelWrapper.addSetup(async (channel: ConfirmChannel) => {
        ...
        this.logger.log(
          `Consumer setup for task.created on queue: ${taskCreatedQueue}`,
        );
        ...
        this.logger.log(
          `Consumer setup for task.updated on queue: ${taskUpdatedQueue}`,
        );
        ...
        this.logger.log(
          `Consumer setup for task.comment.created on queue: ${commentCreatedQueue}`,
        );
      });
  ...
  private handleTaskCreated(msg: ConsumeMessage, channel: ConfirmChannel) {
    try {
      ...
      const event = JSON.parse(content) as { data?: { id?: string } };

      this.logger.log(
        `Received task.created event for task: ${event.data?.id ?? 'unknown'}`,
      );
  ...
  private handleTaskUpdated(msg: ConsumeMessage, channel: ConfirmChannel) {
    try {
      ...
      const event = JSON.parse(content) as { data?: { id?: string } };
      this.logger.log(
        `Received task.updated event for task: ${event.data?.id ?? 'unknown'}`,
      );
  ...
  private handleCommentCreated(msg: ConsumeMessage, channel: ConfirmChannel) {
    try {
      ...
      this.logger.log(
        `Received task.comment.created event for comment: ${event.data?.id ?? 'unknown'}`,
      );
