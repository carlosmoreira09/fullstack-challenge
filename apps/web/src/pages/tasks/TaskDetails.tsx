...
import type {Task, TaskComment, TaskHistoryEntry,UpdateTaskDto} from '@/types';
...
function TaskDetails() {
  ...
  const [comments, setComments] = useState<Array<TaskComment>>([])
  const [history, setHistory] = useState<Array<TaskHistoryEntry>>([])
  ...
  useEffect(() => {
    const fetchComments = async () => {
      try {
        ...
        const response = await commentApi.getCommentsByTask(
          taskId,
          currentPage,
          commentsPerPage,
        )
  ...
  const handleUpdateAssignees = async (assigneeIds: Array<string>) => {
    if (!task) return
  ...
  const renderHistoryChange = (entry: TaskHistoryEntry) => {
    switch (entry.action) {
      ...
      case 'ASSIGNEE_REMOVED':
        return (
          <span className="text-muted-foreground">removeu um responsável</span>
        )
      case 'STATUS_CHANGED':
        return (
          <span className="text-muted-foreground">
            ...
            <span className="font-medium text-foreground">
              {entry.oldValue?.status
                ? STATUS_LABELS[
                    entry.oldValue.status as keyof typeof STATUS_LABELS
                  ]
                : 'N/A'}
            </span>{' '}
            para{' '}
            <span className="font-medium text-foreground">
              {entry.newValue?.status
                ? STATUS_LABELS[
                    entry.newValue.status as keyof typeof STATUS_LABELS
                  ]
                : 'N/A'}
            </span>
          </span>
      ...
      case 'PRIORITY_CHANGED':
        return (
          <span className="text-muted-foreground">alterou a prioridade</span>
        )
        ...
        return (
          <span className="text-muted-foreground">
            ...
            <span className="italic text-foreground">
              "
              {entry.newValue?.content
                ? String(entry.newValue.content).substring(0, 50)
                : ''}
              ..."
            </span>
          </span>
      ...
      case 'COMMENT_UPDATED':
        return (
          <span className="text-muted-foreground">editou um comentário</span>
        )
      case 'COMMENT_DELETED':
        return (
          <span className="text-muted-foreground">deletou um comentário</span>
        )
      case 'UPDATED':
      ...
      default:
        return (
          <span className="text-muted-foreground">realizou uma alteração</span>
        )
    }
  }
            ...
            <div className="flex-1">
              ...
              {!isEditingDetails && (
                <div className="mt-2 flex items-center gap-4 text-sm text-muted-foreground">
                  <span>
                    Criado por{' '}
                    {task.createdByData?.name ?? task.createdById ?? '-'}
                  </span>
                  <span>•</span>
          ...
          <div className="space-y-6 lg:col-span-2">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Discussão e atualizações sobre a tarefa
                </CardDescription>
              </CardHeader>
              <CardContent>
