...
export function KanbanBoard() {
  ...
  const fetchTasks = useCallback(
    async (userIdParam?: string, includeAssignedTasks: boolean = false) => {
      try {
        setIsLoading(true)
        setError(null)
        const data =
          userIdParam && userIdParam !== ''
            ? await taskApi.listTasksByUser(userIdParam, includeAssignedTasks)
        ...
        <div className="flex items-center justify-between px-6 py-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Sistema de gestão colaborativo
            </p>
          </div>
              ...
              <Select value={selectedUserId} onValueChange={setSelectedUserId}>
                ...
                <SelectContent>
                  ...
                  {isLoadingUsers ? (
                    ...
                    users
                      .filter((user): user is User & { id: string } =>
                        Boolean(user.id),
                      )
                      .map((user) => (
              ...
              <div className="flex items-center gap-2">
                <Checkbox
                  ...
                  onCheckedChange={(checked) =>
                    setIncludeAssigned(checked === true)
                  }
                />
                <Label
