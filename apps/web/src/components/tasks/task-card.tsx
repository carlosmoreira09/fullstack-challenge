...
export function TaskCard({ task, onDragStart, onDragEnd, onClick }: TaskCardProps) {
    ...
    return (
        ...
        >
            ...
            {/* Task Title */}
            <h4 className="mb-2 font-semibold text-foreground leading-snug text-balance">{task.title}</h4>

            {/* Task Description */}
            {task.description && (
                <p className="mb-4 text-sm text-muted-foreground line-clamp-2 text-pretty">{task.description}</p>
            )}
