...
export class UsersService {
  ...
  async getOneUserProfile(id: string) {
    return await firstValueFrom(
      this.userClient.send('get-one-user-profile', id),
    );
  }
  ...
  async createUser(userData: CreateUserDto) {
    const createAuth = await firstValueFrom(
      this.authClient.send('create-auth', userData),
    );
    if (createAuth) {
      return await firstValueFrom(
  ...
  async updatePassword(id: string, userData: { password: string }) {
    return await firstValueFrom(
      this.authClient.send('update-password', { ...userData, id: id }),
    );
  }

  async updateUser(id: string, userData: UpdateUserDto) {
    return await firstValueFrom(
      this.userClient.send('update-user', { ...userData, id: id }),
    );
  }
}
