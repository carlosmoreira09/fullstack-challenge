...
export class CommentsService {
    ...
    try {
      ...
      if (task) {
        ...
        const notificationRecipients = [task.createdById, ...task.assignees]
          .filter(
            (recipientId, index, self) => self.indexOf(recipientId) === index,
          )
          ...
          async (recipientId) => {
            try {
              ...
              try {
                ...
                Logger.error(
                  `Failed to fetch recipient ${recipientId}:`,
                  error,
                );
              }
              ...
              Logger.log(
                `Comment notification created for user ${recipientId} (${recipientName})`,
              );
            } catch (error) {
              Logger.error(
                `Failed to create comment notification for ${recipientId}:`,
                error,
              );
            }
          },
  ...
  async update(userId: string, updateCommentDto: UpdateCommentDto) {
    ...
    const updatedComment = await firstValueFrom(
      this.taskClient.send('update-comment', {
        ...updateCommentDto,
